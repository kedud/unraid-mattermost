...
if [[ ! -f "/mattermost/perms.txt" ]]; then

	# set permissions for /config volume mapping
	echo "[info] Setting permissions recursively on /config..."
	chown -R "${PUID}":"${PGID}" /mattermost
	chmod -R 775 /mattermost
	echo "This file prevents permissions from being applied/re-applied to /mattermost, if you want to reset permissions then please delete this file and restart the container." > /mattermost/perms.txt

else

	echo "[info] Permissions already set for /mattermost"

fi

# set permissions inside container
chown -R "${PUID}":"${PGID}" /opt/mattermost
chmod -R 775 /opt/mattermost
